...
module.exports = {
    ...
    methods: {
        ...
        async handleSubscriptions(ctx) {
            ...
            let options = {
                // meta: await this.getInternalMeta({ ownerId: ctx.meta.ownerId || this.adminGroupId })
                meta: { ownerId: ctx.meta.ownerId || this.adminGroupId }
            };
            let subscriptions;
            try {
                // if (!options.meta.ownerId) options.meta.ownerId = "CORE"; 
                this.logger.info("call subscriptions", { params, options });
                subscriptions = await this.broker.call(this.services.query + ".subscriptions", params, options);
            } catch (err) {
                this.logger.error("Failed to retrieve subscriptions", { meta: options.meta, params, err });
                return false;
            }
