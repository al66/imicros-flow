         ...
         */
         emit: {
            ...
            async handler(ctx) {
                ...
                ctx.eventName = ctx.params.eventName;

                return this.handleSubscriptions(newContext);
                */
                ctx.eventName = ctx.params.eventName;
                ctx.params = ctx.params.payload;
                return this.handleSubscriptions(ctx);
            }
        }        
            ...
            if (Array.isArray(subscriptions)) {
                for (let i = 0; i < subscriptions.length; i++) {
                    ...
                    try {
                        ...
                        if (!instanceId) {
                            instanceId = uuid();
                            ctx.emit("flow.instance.created", {
                                ownerId: ownerId,
                        ...
                        let contextKey = subscription.attributes?.contextKey ?? subscription.elementId;
                        let params = {
                            instanceId: instanceId,
                            key: contextKey,
                            value: { 
                                payload: ctx.params,
                                meta: _.omit(ctx.meta, ["acl","auth","token","accessToken","user.token","serviceToken","service"])  // clean up meta
                            }
                        };
