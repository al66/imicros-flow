...
module.exports = {
    ...
    methods: {
        ...
        async handleSubscriptions(ctx) {
            ...
            try {
                meta = await this.getInternalMeta({ ownerId })
            } catch (err) {
                this.logger.error("Failed to retrieve access token", { ownerId, meta: ctx.meta, adminGroupId: this.adminGroupId });
                return false;
            }
            let subscriptions;
            try {
                // if (!options.meta.ownerId) options.meta.ownerId = "CORE"; 
                this.logger.info("call subscriptions", { params, meta });
                subscriptions = await this.broker.call(this.services.query + ".subscriptions", params, { meta });
            } catch (err) {
