...
// Kafkajs used for consumer
const { Kafka, logLevel } = require('kafkajs')

// kafka-node used for producer
var kafka, HighLevelProducer;
...
module.exports = {
	...
	methods: {
        ...
        async callAction (subscription, content) {
            ...
            try {
                let result = await this.broker.call(subscription.action, params)
                this.logger.debug(`Action ${subscription.action} called`, { params: params, result: result })
                return result;
            } catch (err) {
                throw err;
            }
        ...
        async emitTerminated (subscription, content, result) {
            ...
            try {
              ...
              await new Promise((resolve, reject) => {
                ...
                this.logger.debug(`Emit event ${content.event} to topic ${this.topics.events}`, content)
            } catch(err) {
