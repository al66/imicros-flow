...
module.exports = {
    ...
    actions: {
        ...
        handle: {
            ...
            async handler(ctx) {
                ...
                let meta = this.getMeta({ token: token });
                let current = this.getElement({ token: token });
                if ( !current ) {
                    return true;    // ignore token
                }
         ...
         * get exchange token
         *
         * exchange access token at acl service against token for access on behalf of the user/owner
         * 
         * @actions
        ...
        getExchangeToken: {
            ...
            async handler(ctx) {
                ...
                try {
                    exchangeToken = await this.broker.call(this.services.acl + ".exchange", params, {});
                } catch (err) {
                    this.logger.error("Failed to exchange access token", { accessToken: params.accessToken, processId: params.processId, instanceId: params.instanceId });
                }
    ...
    methods: {
        ...
        async getElement({ token }) {
            let element;
            let opts = {
                meta: this.getMeta({ token: token })
            };
            try {
                element = await this.broker.call(this.services.query + ".get", { processId: token.processId, elementId: token.elementId }, opts);
            } catch (err) {
                this.logger.error("Failed to retrieve details of element", { token: token });
            }
            return element;
        },
