...
module.exports = {
    ...
    methods: {
        ...
        async subscribe (subscription) {
            try {
                ...
                let consumer = this.kafka.consumer({ groupId: subscription.id });
								...
								/* TEST */
                //consumer.on(consumer.events.COMMIT_OFFSETS,() => {console.log("Consumer " + subscription.id + " COMMIT_OFFSETS")})
								//consumer.on(consumer.events.HEARTBEAT,() => {console.log("Consumer " + subscription.id + " HEARTBEAT")})
            ...
            return async ({ topic, message }) => {
                ...
                try {
                    ...
                    if (subscription.emit) {
                        let payload =  subscription.emit.payload ? subscription.emit.payload : result;
                        if (subscription.emit.payload) {
                            try {
                                payload =  mapper(payload, { meta: content.meta, payload: content.payload, result: result });
                            } catch (err) {
                                this.logger.debug("Error mapping parameters", { payload: subscription.emit.payload, error: err });
                            }
                        }
                        this.emit({
                            topic: topic,
        ...
        async emit ({topic, offset, subscription, event, payload, meta}) {
            ...
            try {
                await this.producer.send({
                    topic: subscription.emit.topic,
                    messages: [
                            { value: JSON.stringify(content) }
                    ],
                });
                this.logger.debug(`Emitted event for consumed event topic ${topic} offset ${offset}`, {
                ...
                this.logger.error(`Failed to emit event for consumed event topic ${topic} offset ${offset}`, {
                    subcription: subscription.id,
                    event: event,
                    payload: payload,
                    meta: meta,
    ...
    async started() {
        ...
        if (this.settings.subscriptions && Array.isArray(this.settings.subscriptions) ) {
            await Promise.all(this.settings.subscriptions.map(async subscription => {
                ...
                subscription.topic = subscription.topic || this.topics.events;

								// Static descriptions have access to events all emitted events
                subscription.meta = {
                    access: Array.isArray(subscription.access) ?  subscription.access : [ Constants.STATIC_GROUP ]
                };
    ...
    async stopped() {
        ...
        try {
            //await Promise.all(this.consumers.map(async (consumer)=> await consumer.pause([{topic: this.topics.events}])));
