...
module.exports = {
        ...
        async handle({ token }) {
            switch ( token.status ) {
                ...
                case Constants.EVENT_ACTIVATED:
                    await this.broker.call(this.services.event + ".activated", { token });
                    break;
                ...
                case Constants.SEQUENCE_ACTIVATED:
                    await this.broker.call(this.services.sequence + ".activated", { token });
                    break;
                ...
                case Constants.ACTIVITY_ACTIVATED:
                    ...
                    await this.broker.call(this.services.activity + ".activated", { token });
                    break;
                case Constants.ACTIVITY_READY:
                    // get activity attributes and execute activity
                    // if executed w/o errors emit completed token
                    await this.broker.call(this.services.activity + ".ready", { token });
                    break;
                ...
                case Constants.GATEWAY_ACTIVATED:
                    await this.broker.call(this.services.gateway + ".activated", { token });
                    break;
                case Constants.GATEWAY_READY:
                    ...
                    await this.broker.call(this.services.gateway + ".ready", { token });
                    break;
    ...
    async created() { 
        ...
        this.services = {
            event: _.get(this.settings,"services.event","flow.event"),
            gateway: _.get(this.settings,"services.gateway","flow.gateway"),
            sequence: _.get(this.settings,"services.sequence","flow.sequence"),
            activity: _.get(this.settings,"services.activity","flow.activity")
