...
class DB {
    ...
    async saveProcess({ opts, xmlData, parsedData }) {
        try {
            ...
            await parsedData.event.forEach(async (event) => {
                if (event.position === Constants.START_EVENT || ( event.position === Constants.INTERMEDIATE_EVENT && event.direction === Constants.CATCHING_EVENT) ) {
    ...
    async getProcess({ opts, processId, versionId, xml = null }) {
        try {
            ...
            // query version table
            let query = "SELECT deployed, parsed, oek, iv FROM " + this.versionTable;
            if (xml) {
                query = "SELECT deployed, parsed, xml, oek, iv FROM " + this.versionTable;
            }
            query += " WHERE owner = :owner AND process = :process AND version = :version;";
            let params = { 
                owner, 
                process: processId, 
                version: versionId
    ...
    async getVersionList({ opts, processId }) {
        try {
            ...
            if (processId) {
                query += " WHERE owner = :owner AND process = :process;";
            } else {
    ...
    async activateVersion({ opts, processId, versionId }) {
        try {
            ...
            // update active table
            let query = "UPDATE " + this.activeTable +" SET active["+ processId +"] = " + versionId;
            query += " WHERE owner = :owner;";
            let params = { 
                owner
            };
    ...
    async deactivateProcess({ opts, processId }) {
        try {
            ...
            // update active table
            let query = "DELETE active["+ processId +"] FROM " + this.activeTable;
            query += " WHERE owner = :owner;";
            let params = { 
                owner
            };
