...
module.exports = {
    ...
    methods: {
                        ...
                        case "ruleset": 
                            {
                                ...
                                this.logger.info("Parameter preparation in service task executed", { ruleset, context, result, opts });
                            }
                            break;
                    }
        ...
        async handleCompleted({ token, result, error }) {
            ...
            let resultKey = activity?.attributes?.resultKey ?? token.elementId;
            // save result to context
            if (result) {
                await this.broker.call(this.services.context + ".add", { instanceId: token.instanceId, key: resultKey, value: result }, opts);
            }
